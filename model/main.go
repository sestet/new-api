...
func InitDB() (err error) {
	...
	if err == nil {
		...
		common.SysLog("database schema initialization started")
...
func InitLogDB() (err error) {
	...
	if err == nil {
		...
		common.SysLog("log database schema initialization started")
		err = initializeLogDBSchema()
		return err
	} else {
		common.FatalLog(err)
	}
...
func initializeDBSchema() error {
	...
	if hasExistingTable {
		for _, schemaModel := range models {
			if !DB.Migrator().HasTable(schemaModel) {
				return fmt.Errorf("database schema is incomplete: missing %T; rebuild the development database from the current baseline", schemaModel)
			}
		}
	} else if err := DB.Migrator().CreateTable(models...); err != nil {
		return err
	}
	return nil
}

func initializeLogDBSchema() error {
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		return initializeClickHouseLogDB()
	}
	if LOG_DB.Migrator().HasTable(&Log{}) {
		return nil
	}
	return LOG_DB.Migrator().CreateTable(&Log{})
}

func initializeClickHouseLogDB() error {
