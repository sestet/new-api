...
type UpstreamBillingProvider string

const (
	UpstreamBillingProviderAuto    UpstreamBillingProvider = "auto"
	UpstreamBillingProviderNewAPI  UpstreamBillingProvider = "new_api"
	UpstreamBillingProviderSub2API UpstreamBillingProvider = "sub2api"
)

type UpstreamBillingSettings struct {
	CredentialID           int                     `json:"credential_id,omitempty"`
	Proxy                  string                  `json:"-"`
	Enabled                bool                    `json:"enabled,omitempty"`
	Provider               UpstreamBillingProvider `json:"provider,omitempty"`
	AccessToken            string                  `json:"access_token,omitempty"`
	AccessTokenConfigured  bool                    `json:"access_token_configured,omitempty"`
	RefreshToken           string                  `json:"refresh_token,omitempty"`
	RefreshTokenConfigured bool                    `json:"refresh_token_configured,omitempty"`
	...
	DetectedProvider       UpstreamBillingProvider `json:"detected_provider,omitempty"`
	RecheckEnabled         *bool                   `json:"recheck_enabled,omitempty"`
...
func (s *UpstreamBillingSettings) IsRecheckEnabled() bool {
	return s != nil && (s.RecheckEnabled == nil || *s.RecheckEnabled)
}

func (s *UpstreamBillingSettings) RecheckWindow() time.Duration {
	hours := 24
	if s != nil && s.RecheckWindowHours > 0 {
		hours = s.RecheckWindowHours
	}
	return time.Duration(hours) * time.Hour
}

func (s *UpstreamBillingSettings) NormalizeCredentialTypes() bool {
	if s == nil || s.Provider == UpstreamBillingProviderNewAPI || s.RefreshToken != "" || !strings.HasPrefix(s.AccessToken, "rt_") {
		return false
	}
	s.RefreshToken = s.AccessToken
	s.RefreshTokenConfigured = false
	s.AccessToken = ""
	s.AccessTokenConfigured = false
	s.AccessTokenIssuedAt = 0
	s.AccessTokenExpiresAt = 0
	if s.Provider == "" || s.Provider == UpstreamBillingProviderAuto {
		s.DetectedProvider = UpstreamBillingProviderSub2API
	}
	return true
}

func (s *UpstreamBillingSettings) Validate() error {
	if s == nil {
		return nil
	}
	...
	s.NormalizeCredentialTypes()
	if s.CredentialID < 0 {
		return fmt.Errorf("upstream billing credential ID cannot be negative")
	}
	...
	if s.Provider == "" {
		s.Provider = UpstreamBillingProviderAuto
	}
	switch s.Provider {
	case UpstreamBillingProviderAuto, UpstreamBillingProviderNewAPI, UpstreamBillingProviderSub2API:
	default:
		return fmt.Errorf("unsupported upstream billing provider: %s", s.Provider)
	}
	if s.DetectedProvider != "" && s.DetectedProvider != UpstreamBillingProviderNewAPI && s.DetectedProvider != UpstreamBillingProviderSub2API {
		return fmt.Errorf("unsupported detected upstream billing provider: %s", s.DetectedProvider)
	}
	if s.UserID < 0 {
		return fmt.Errorf("upstream billing user ID cannot be negative")
	}
	switch s.Provider {
	case UpstreamBillingProviderNewAPI:
		if s.AccessToken == "" && !s.AccessTokenConfigured {
			return fmt.Errorf("upstream billing access token is required")
		}
	case UpstreamBillingProviderSub2API:
		if s.RefreshToken == "" && !s.RefreshTokenConfigured && s.AccessToken == "" && !s.AccessTokenConfigured {
			return fmt.Errorf("sub2api refresh token is required")
		}
	case UpstreamBillingProviderAuto:
		if s.AccessToken == "" && !s.AccessTokenConfigured && s.RefreshToken == "" && !s.RefreshTokenConfigured {
			return fmt.Errorf("upstream billing account credential is required")
		}
	}
	s.APIBaseURL = strings.TrimSpace(s.APIBaseURL)
	if s.APIBaseURL == "" {
		return nil
	}
	parsed, err := url.ParseRequestURI(s.APIBaseURL)
	if err != nil || parsed.Host == "" || parsed.User != nil || (parsed.Scheme != "http" && parsed.Scheme != "https") {
		return fmt.Errorf("invalid upstream billing API base URL")
	}
	if parsed.RawQuery != "" || parsed.Fragment != "" {
		return fmt.Errorf("upstream billing API base URL cannot contain query or fragment")
	}
	return nil
}
...
type ChannelOtherSettings struct {
	...
	UpstreamBilling                       *UpstreamBillingSettings `json:"upstream_billing,omitempty"`
