...
func refreshTieredBillingGroup(relayInfo *relaycommon.RelayInfo) (*billingexpr.BillingSnapshot, error) {
	...
	snap.GroupRatioSource = relayInfo.PriceData.GroupRatioInfo.Source
	if snap.GroupRatioSource == "" {
		snap.GroupRatioSource = "group_ratio"
	}
	snap.EstimatedQuotaAfterGroup = estimatedQuota
	snap.EstimatedQuota = estimatedQuota
	return snap, nil
}
...
func TryTieredSettle(relayInfo *relaycommon.RelayInfo, params billingexpr.TokenParams) (ok bool, quota int64, result *billingexpr.TieredResult) {
	...
	// Surface any quota saturation from settlement onto RelayInfo so the
