...
export function processChartData(
  ...
  chartTimes.forEach((time) => {
    let timeData = sortedModels.map((model) => {
      ...
      const usage = usd ? Number(usd.toFixed(8)) : 0
      return {
        Time: time,
        Model: model,
        rawQuota,
  ...
  chartTimes.forEach((time) => {
    ...
    sortedModels.forEach((model) => {
      ...
      buckets.set(key, {
        ...
        usage: Number((prev.usage + usage).toFixed(8)),
      })
    })
...
export function processUserChartData(
  ...
  const rankValues = sorted.slice(0, limit).map(([username, quota]) => ({
    User: username,
    rawQuota: quota,
    Usage: Number((quota / quotaPerUnit).toFixed(8)),
  }))
  ...
  sortedTimePoints.forEach((time) => {
    topUsers.forEach((user) => {
      ...
      trendValues.push({
        ...
        User: user,
        rawQuota: q,
        Usage: Number((q / quotaPerUnit).toFixed(8)),
      })
    })
  })
